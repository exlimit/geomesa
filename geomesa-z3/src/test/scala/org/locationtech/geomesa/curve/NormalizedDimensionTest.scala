...
class NormalizedDimensionTest extends Specification {
  ...
  "NormalizedDimension" should {
    ...
    "Round-trip normalize maximum" >> {
      NormLat.normalize(NormLat.denormalize(precision.toInt)) mustEqual precision - 1
      NormLon.normalize(NormLon.denormalize(precision.toInt)) mustEqual precision - 1
    }
    ...
    "Normalize maximum" >> {
      NormLat.normalize(NormLat.max) mustEqual precision - 1
      NormLon.normalize(NormLon.max) mustEqual precision - 1
    }
...
//    }
    ...
    "Denormalize [1,precision] to bin middle" >> {
      ...
      NormLat.denormalize(0) must beCloseTo(NormLat.min + latWidth / 2d, epsilon)
      NormLat.denormalize(precision.toInt - 1) must beCloseTo(NormLat.max - latWidth / 2d, epsilon)

      val lonExtent = NormLon.max - NormLon.min
      val lonWidth = lonExtent / precision
      NormLon.denormalize(0) must beCloseTo(NormLon.min + lonWidth / 2d, epsilon)
      NormLon.denormalize(precision.toInt - 1) must beCloseTo(NormLon.max - lonWidth / 2d, epsilon)
    }
